...
void Renderer::SetPixel(int x, int y, uint32_t argb)
{
...
void Renderer::DrawEllipse(int x0, int y0, int xRadius, int yRadius, uint32_t argb)
{
    ny(y0);
...
void Renderer::DrawRect(int x0, int y0, int width, int height, uint32_t argb)
{
    int x1, y1;
    ny(y0);
    x1 = x0 + width;
...
void Renderer::DrawTriangle(int x0, int y0, int x1, int y1, int x2, int y2, uint32_t argb)
{
    ny(y0);
    ny(y1);
    ny(y2);
...
void Renderer::ny(int& y)
{
    y = Height() - y;
}

void Renderer::SetBackgroundColor(uint8_t r, uint8_t g, uint8_t b)
{
    m_background.r = r;
    m_background.g = g;
    m_background.b = b;
...
std::shared_ptr<Renderer> Renderer::Create(SDL_Window *window, unsigned int width, unsigned int height)
{
    return std::shared_ptr<Renderer>(static_cast<Renderer*>(new Renderer(window, width, height)));
}
